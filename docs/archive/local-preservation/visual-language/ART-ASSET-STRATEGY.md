...
For stateful theory surfaces, pair this asset decision with
`INTERACTION-GRAMMAR.md`: artwork may be a field, crop, fragment or state
layer while live HTML and theory-specific interaction carry the explanation.
...
Authored artwork is also a vocabulary library: reuse a meaningful fragment,
figure, waveform or source detail when a complete composition would compete
with the content. See `DESIGN-SYSTEM.md` for intensity and placement rules.
