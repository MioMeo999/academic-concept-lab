...
export function TonalBody({ record: r }: { record: TheoryRecord }) {
  ...
  add("provenance", "Where every claim came from", "var(--teal)", <div className="prov tonal-provenance">{r.provenance.map((item) => <div className="prov-item" key={item.label} data-reveal="rise"><span className="g" style={{ color: item.colour }} aria-hidden="true">{item.glyph}</span><div><h3>{item.label}</h3><p>{item.note}</p></div></div>)}</div>);
