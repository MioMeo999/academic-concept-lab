...
function ImplicationCompare({ data }: { data: NarmourRecordContent }) {
  ...
  return (
    <div className="narmour-interaction">
      ...
      <details className="narmour-static-fallback" open><summary>Read the fixed stimulus</summary><p>Triangle timbre · 0.12 gain · 420 ms tone duration · 600 ms onset-to-onset IOI · C4-centered register · no backing harmony or probe-timing manipulation.</p></details>
    </div>
  );
}
...
export function NarmourBody({ record: r }: { record: TheoryRecord }) {
  ...
  add("opening", "Continue — or reverse?", "Continue — or reverse?", "var(--red)", <><Rich className="lede" as="p" html={data.opening.lede} /><p className="narmour-question">{data.opening.question}</p><Note glyph="?">{data.opening.note}</Note><ImplicationCompare data={data} /><Note glyph="▲">{data.analysisNote}</Note></>);
  ...
  add("scope", "What I–R explains — and where it stops", "What I–R explains — and where it stops", "var(--teal)", <><Rich className="lede" as="p" html={data.scope.lede} /><FinalModel /><div className="narmour-scope-grid"><div className="sk-box teal"><p className="k">I–R helps explain</p><List items={data.scope.explains} /></div><div className="sk-box red"><p className="k">Where I–R stops</p><List items={data.scope.stops} colour="var(--red)" /></div></div><Note glyph="?">{data.scope.note}</Note></>);
