...
export function StatisticalBody({ record: r }: { record: TheoryRecord }) {
  ...
  add("provenance", "Provenance", "Where every claim came from", "var(--teal)", <div className="prov stat-provenance">{r.provenance.map((item) => <div className="prov-item" key={item.label}><span className="g" style={{ color: item.colour }} aria-hidden="true">{item.glyph}</span><div><h3>{item.label}</h3><p>{item.note}</p></div></div>)}</div>);
